...
def transition(v_p, o_p, bit_rate ,ffmpeg_path):
    """v_p:源文件位置;o_p:输出目录;bitspeed:码率"""
    f_n = ''.join(v_p.split('\\')[-1].split('.')[0:-1])  # 文件名
    f_m = v_p.split('\\')[-1].split('.')[-1]  # 文件扩展名
    o_p = o_p + '\\' + f_n + "[NEW]." + f_m
    t1 = time.time()
    os.system(f'{ffmpeg_path} -i "{v_p}" -vcodec libvpx-vp9 -c:a copy -b:v {bit_rate} -pass 1 -an -sn "{o_p}"')
    os.system(f'{ffmpeg_path} -i "{v_p}" -vcodec libvpx-vp9 -c:a copy -b:v {bit_rate} -pass 2 -y "{o_p}"')
    t2 = time.time()
    print(f"用时{t2 - t1}")
...
def main_method():
    ...
    if f_l == 0:
        ...
        for i in f_l:
            transition(i, out_p, '1500k', ffmpeg_path)
    end_ = input("end!")
